...
def generate_pdf_report(file_name, json_data):
    logger.info("--- ENTERING generate_pdf_report FUNCTION (reportlab) ---")
    output_filename = f"{Path(file_name).stem}_report.pdf"
    ...
    try:
        ...
        report_data = [
            ("Threat Level", json_data.get("threat_level", "N/A")),
            ...
            ("MITRE ATT&CK Mapping", ", ".join(json_data.get("mitre_mapping", [])) or "N/A"),
            ("Recommended Actions", "\n".join([f"- {action}" for action in json_data.get("recommended_actions", [])]) or "N/A"),
        ]
        ...
        story.append(Paragraph(f"Report generated on: {datetime.now().strftime('%Y-%m-%d %H:%M:%S')} using Gemini AI", styles['Italic']))
